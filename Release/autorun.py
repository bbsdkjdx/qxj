...
def refresh_actives(x, y):
    ...
    for n, x in enumerate(actives):
        #__main__.msgbox(x[-2])
        # -1 is sta,-2 is server actives number.
        __main__.exe_fun__['append_actives'](n, x[0], x[1], x[2], x[-1], x[-2])
...
def get_history(x, y):
    ...
    for n, x in enumerate(sorted(summary.keys(), key=skey)):
        ...
        s4 = "%d次，%d小时" % (
            summary[x][0]+summary[x][2], summary[x][1]+summary[x][3])
